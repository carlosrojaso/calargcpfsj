...
prx.types.toolbar = {
	...
	,onDisplay: function(item,containerid) {
		...
		var cReturn = '<div id="'+_id+'" class="box pos type-toolbar" '+((item.overlay)? 'data-mpoverlay="1"': '')+'><div class="inner liveUpdate-backgroundColor changeProperty-backgroundColor" style="background-color:'+getColor(item.backgroundColor)+';"></div></div>';
		return cReturn;
	}	
	,propertyGroups:
		[			
...
prx.types.tabbar = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		$.each(item.tabs, function(i,elm){
			...
			cR += '<li id="'+_id+'-tabs-'+i+'" style="width: '+_width+'px;" class="dynamic-property" data-dynamic-property-index="'+i+'" '+((elm.linkedscreen != -1) ? 'data-linked-screen="'+elm.linkedscreen+'"' : '')+'>';
			cR += '<input type="radio" name="'+_id+'-radio" id="'+_id+'-radio-'+i+'"'+cChecked+' data-role="none" '+((!item.changeState) ? 'disabled' : '')+'/>';
...
prx.types.button2 = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		case 'right':
			...
			content += 'right: '+content_left+'px; border-width: '+item.borderWidth+'px; width: '+content_width+'px;';
			content += cssBorderRadius(item.borderRadius + 'px'); 
			break;
		case 'left':
			arr += 'left: '+arrow_pos+'px;';
			content += 'left: '+content_left+'px; border-width: '+item.borderWidth+'px; width: '+content_width+'px;';
			content += cssBorderRadius(item.borderRadius + 'px');
			break;
		case 'none':
		default: 
		...
		case 'right':
			if(item.buttonicon.url != '') {
				cR = cR + '<img class="floated-img" src="'+getAssetUrl(item.buttonicon)+'" style="'+item.iconpos+': 8px; height: '+eval((_dims.height-item.borderWidth*2)*0.2*item.iconSize)+'px; padding-top: '+eval((_dims.height-item.borderWidth*2)*0.1*(5-item.iconSize))+'px; " />';
			}
			break;
		case 'notext':
		...
		if(item.arrowDirection == 'right') {
			cR = cR + '<div class="button-arrow" style="overflow: hidden; height: 100%; width: '+Math.round(_dims.height/2)+'px; '+item.arrowDirection+': 0;"><div class="liveUpdate-borderColor liveUpdate-backgroundColor changeProperty-borderColor changeProperty-borderWidth changeProperty-backgroundColor" style="height: '+arrow_h+'px; width: '+arrow_h+'px; '+arr+' top: '+arrow_pos+'px; '+bgArrCss+'"></div></div>';
		}
		cR = cR + '</div>';
		cR = cR + '</div>';
		return cR;
...
prx.types.segmentedcontrol = {
	name: "segmentedcontrol"
		...
		$.each(item.options, function(i,elm){
			...
			cR += '<li style="'+_spanstyle+'" id="'+_id+'-options-'+i+'"><input type="radio" name="'+_id+'-radio" id="'+_id+'-radio-'+i+'"'+_active+' data-role="none" '+((!item.changeActive) ? 'disabled' : '')+'/>';
...
prx.types.pagecontroller = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		$.each(item.buttons, function(i,elm){
			...
			cR += '<input type="radio" data-role="none" name="'+_id+'-radio" id="'+_id+'-option-'+i+'" '+_checked+' '+((!item.changeActive) ? 'disabled' : '')+'/>'
...
prx.types.radiobutton = {
	...
	,onDisplay: function(item,containerid) {
		...
		cR += '<label class="liveUpdate-backgroundColor changeProperty-backgroundColor" for="'+_id+'-checkbox" style="'+_bg+'" for="'+_id+'-checkbox" style="'+_bg+'"><span class="liveUpdate-backgroundColor changeProperty-backgroundColor" style="'+_bg+'"></span></label>';
		cR += '</div>';
		return cR;
	}
	,afterDisplay: function(item,containerid) {
...
prx.types.checkbox = {
	...
	,onDisplay: function(item,containerid) {
		...
		cR += '<label for="'+_id+'-checkbox" class="liveUpdate-backgroundColor changeProperty-backgroundColor" style="'+_bg+' display: block; width: 100%; height: 100%;">'+_check+'</label>';
		cR += '</div>';
		return cR;
	}
	,afterDisplay: function(item,containerid) {
...
prx.types.checkboxlist = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		$.each(item.checkboxes, function(i,elm){
			...
			cR += '<label data-dynamic-property-index="'+i+'" class="dynamic-property liveUpdate-borderColor changeProperty-borderColor changeProperty-activeBackgroundColor changeProperty-activeTextColor '+((elm.active) ? 'liveUpdate-activeBackgroundColor liveUpdate-activeTextColor' : '')+'" for="'+_id+'-checkbox-'+i+'" style="'+_style+' border-color: '+getColor(item.borderColor)+';"><span data-editableproperty="text" data-dynamic-property-index="'+i+'">' +elm.text+ '</span>' + _checkmark + '</label>'
			cR += '</span>';
		});
		cR += '</div></div>';		
		return cR;
...
prx.types.picker = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		cR += '<div class="inner liveUpdate-listColor liveUpdate-textColor changeProperty-textColor changeProperty-textFont" id="' + _id + '-inner" style="background-color: '+getColor(item.listColor)+'; color: '+getColor(item.textColor)+'; '+getFontCssFromFontFamily(item.textFont)+' '+_props+'">';
			...
			//if (typeof(prx.scroller[_id + '-inner'])=='undefined') {
				try {
					...
					prx.scroller[_id + '-inner'].on('scrollEnd', function() {
						...
						var index = Math.round(prx.scroller[_id + '-inner'].y/32) * -1;
						
						prx.scroller[_id + '-inner'].scrollTo(0,index*-32);
					...
					*/
					...
					if(item.selectedValue <= _options.length && item.selectedValue!=-1) {
						prx.scroller[_id + '-inner'].scrollTo(0,-item.selectedValue * 32);
					}
				} catch(err){};
			//}
		} else {
...
prx.types.listcomplex = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		$.each(item.listitems, function(i,elm){
			...
			/* THUMBNAIL */
			//if(elm.hasThumbnail && typeof(elm.thumbnail) != "undefined" &&  elm.thumbnail.url != '') {
...
prx.types.actionsheet = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		var cR = '';
		cR += '<div id="' + _id + '" class="box pos type-actionsheet"><div  class="actionsheet-inner liveUpdate-backgroundColor changeProperty-backgroundColor"  style="padding: '+item.padding+'px; '+_bg+' background-size: 100% 20px; background-repeat: no-repeat; background-color: '+getColor(item.backgroundColor)+';">';
		$.each(item.buttons, function(i,elm){
...
prx.types.popover = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		cR += '<div class="popover-content liveUpdate-backgroundColor liveUpdate-borderColor-border-color changeProperty-backgroundColor" style="height: '+_height2+'; '+_borderRadius+' border: 5px solid '+getColor(item.borderColor)+'; background-color: '+getColor(item.backgroundColor)+'"></div>';
		cR += '</div></div>';
